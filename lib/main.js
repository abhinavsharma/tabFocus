...
const tabFocus = new focus.TabFocus();
...
let galleryPanel = panel.Panel({
  ...
  onShow: function() {
  // spinQuery for the right folders
   this.postMessage({
      "folders" : getBookmarkFolders(),
    });
  },
  onMessage: function(data) {
    console.log("got message for id" + data.id);
    //spinQuery for the right urls
