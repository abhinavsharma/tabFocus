...
function recall() {
  ...
  function initTabs() {
    ...
    if(moveTabs.length == 0) {
      currentWindow.TabView.getContentWindow().GroupItems.setActiveGroupItem(newGroup);
    }
  }
...
widgets.Widget({
  ...
  contentURL: self.data.url("img/group.png"),
  onClick: function() {
    recall();
  }
});
...
let mod = pageMod.PageMod({
  include: "resource://tabfocus-at-prospector-dot-labs-dot-mozilla-tabfocus-data/*",
  contentScriptFile: self.data.url("monkey.js"),
  onAttach: function attached(worker) {
    console.log("attaching, have clusters: " + clusters);
    worker.postMessage({
      "clusters" : clusters,
    });
